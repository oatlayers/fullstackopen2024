...
describe("Blog app", () => {
  ...
  describe("Login", () => {
    ...
    describe("When logged in", () => {
      ...
      test("a new blog can be created", async ({ page }) => {
        ...
        await expect(
          page.getByRole("link", { name: "oatlayers.com" })
        ).toBeVisible();
      });
      ...
      test("blog can be edited", async ({ page }) => {
        await createBlog(
          page,
          "test blog edit like by playwright",
          "oatlayers6",
          "oatlayers.com",
          true
        );
      ...
      test("user who added the blog can delete the blog", async ({ page }) => {
        ...
        page.on("dialog", async (dialog) => {
          expect(dialog.message()).toEqual(
            "Remove blog test delete by playwright by oatlayers6?"
          );
          await dialog.accept();
        });
  ...
  test("blogs are arranged in the order according to the likes", async ({
    ...
    await page
      .locator("div")
      .filter({ hasText: /^test blog order three oatlayers6view$/ })
      .locator("#view-button")
      .click();
  ...
  test("only user who added the blog can delete the blog", async ({
    page,
    request,
  }) => {
    await request.post("/api/users", {
    ...
    await createBlog(
      page,
      "test only user delete by playwright",
      "oatlayers5",
      "oatlayers.com",
      true
    );
