...
test('verifies post request is successful', async () => {
  ...
  const res = await api.post('/api/login').send(toLogin).expect(200)
  ...
  expect(blogsTitle).toContain('Test POST request blog')
})
...
describe('creating users', () => {
  test('invalid username and password length are not created', async () => {
    ...
    expect(result.body.error).toContain(
      'both username and password must be at least 3 characters in length',
    )
  })
